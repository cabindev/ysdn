...
if (empty($emailOrName) || empty($password)) {
    header("Location: /ysdn/app/auth/login.php?msg=missing_fields");
    exit;
}
...
if (RateLimiter::isBlocked($identifier)) {
    ...
    header("Location: /ysdn/app/auth/login.php?msg=rate_limited&wait={$minutes}");
    exit;
}
...
if (!$user_obj->checkEmailOrNameExists($emailOrName)) {
    ...
    header("Location: /ysdn/app/auth/login.php?msg=not_registered");
    exit;
}
...
header("Location: /ysdn/app/auth/login.php?msg=incorrect_password");
exit;
