...
// Set the initial display status of the form container 
$displayForm = isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit' ? 'block' : 'none';
$switchSliderColor = $displayForm == 'block' ? 'green' : 'gray';
?>
<!DOCTYPE html>
<html lang="en">
<head>
	<?php require $_SERVER['DOCUMENT_ROOT']."/ysdn/app/inc/components/nav.php";?>
</head>		
<body class="container">
	<div class="row mt-5">
		<div class="col">
			<div class="card m-3">
				<div class="card-header  text-black d-flex justify-content-between m-3">
					<h4>แบบฟอร์ม<?php echo (isset($_REQUEST['action'])=='edit') ? "แก้ไขข้อมูลสมาชิก" : "เพิ่มข้อมูลส่วนตัว";?></h4>
					<div class="switch-container">
						<input type="checkbox" id="switch" onchange="toggleForm()">
						<label class="switch-slider" for="switch" id="switch-slider"></label>
					</div>
				</div>
				
				<div class="container form-container" id="form-container">
					<form class="row g-3" action="saveMember.php" method="POST" id="myForm" enctype="multipart/form-data">
					<?= csrf_field() ?>
                    <input type="hidden" name="action" value="<?php echo (isset($_REQUEST['action'])=='edit') ? "edit" : "add";?>">
                        ...
                        <div col-md-2>
						<button class="btn btn-success text-white m-3" type="submit">บันทึก</button>
                        </div>
					</form>
				</div>
			</div>
